		...
		<div class="container">
			...
			<section id="container">
				<form role="form" method="get">
						...
						<div class="col-xs-3 col-sm-3" style="margin-left: 3px; margin-right: -25px;">	
							<select name="searchType" class="form-control">
								<option value="n"<c:out value="${scri.searchType == null ? 'selected' : ''}"/>>-----</option>
								<option value="t"<c:out value="${scri.searchType eq 't' ? 'selected' : ''}"/>>사업명</option>
								<option value="c"<c:out value="${scri.searchType eq 'c' ? 'selected' : ''}"/>>내용</option>
								<option value="w"<c:out value="${scri.searchType eq 'w' ? 'selected' : ''}"/>>작성자</option>
								<option value="tc"<c:out value="${scri.searchType eq 'tc' ? 'selected' : ''}"/>>사업명+내용</option>
							</select>
						</div>
					...
					<table class="table table-hover">
						<thead>
							<tr><th style="width:60px; text-align: center;">번호</th>
							<th style="width:650px; text-align: center;">사업명</th>
							<th style="width:100px; text-align: center;">작성자</th>
							<th style="width:120px; text-align: center;">공고일자</th>
							<th style="width:80px; text-align: center;">조회수</th></tr>
						</thead>
							<tr>
								<td style="text-align: center;">1</td>
								<td style="text-align: center;">사업1</td>
					...
					<div class="col-md-offset-3"  style="text-align:center; margin-left: 0px;>
						<ul class="pagination"">
							<c:if test="${pageMaker.prev}">
								<li><a href="list2${pageMaker.makeSearch(pageMaker.startPage - 1)}">이전</a></li>
							</c:if> 
							
							<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
								<li <c:out value="${pageMaker.cri.page == idx ? 'class=info' : ''}" />>
								<a href="list2${pageMaker.makeSearch(idx)}">${idx}</a></li>
							</c:forEach>
							
							<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
								<li><a href="list2${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
							</c:if> 
						</ul>
					</div>
				</form>
